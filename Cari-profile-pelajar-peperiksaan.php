...
p.matrik, ps.kodstatus_pel, sp.namastatus_MY, sp.namastatus_EN
FROM
pel_semester ps
inner join pel_pelajar p on (ps.matrik = p.matrik)
inner Join pilih_status sp ON (ps.kodstatus_pel = sp.kodstatus)
WHERE
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        ...
                                        do {
                                            ...
                                            $semester         = intval($row_rsPljrSem['semester']);
                                            ...
                                            if ($sesi_sem == 'PK/PM' || $sesi_sem == 'SPECIAL EXAM') {
                                                $query_rstran = sprintf("SELECT id_trans, semester, png, pngk, kptsan_sms, status_view, jana_slip, prog_id
	                                                FROM pel_transkrip WHERE id_trans = '$id_trans'");
                                            } else {
                                                $query_rstran = sprintf("SELECT id_trans, semester, png, pngk, kptsan_sms, status_view, jana_slip, prog_id
	                                                FROM pel_transkrip WHERE matrik = '$matrik' AND sesi_sem='$sesi_sem' and semester='$semester' and aktif='1'");
                                            }
